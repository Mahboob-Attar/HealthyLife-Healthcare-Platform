...
symptoms_dict = {
    ...
    'yellow_urine': 42, 'yellowing_of_eyes': 43, 'acute_liver_failure': 44, 'fluid_overload': 45,
...
def predict():
    ...
    dis_des, my_precautions, med_list, diet_list, wrkout_list = helper(predicted_disease)

    return jsonify({
        "status": "success",
        "prediction": predicted_disease,
        "description": dis_des if dis_des else "No description available",
        "precautions": my_precautions if my_precautions else ["No data available"],
        "medications": med_list if med_list else ["No data available"],
        "diets": diet_list if diet_list else ["No data available"],
        "workouts": wrkout_list if wrkout_list else ["No data available"]
    })